...
let UserService = class UserService {
    ...
    save(user) {
        return rxjs_1.from(this.userRepository.save(this.encryptPassword(user)));
    }
    findOne(user) {
        return rxjs_1.from(this.userRepository.findOne(this.encryptPassword(user)));
    }
    encryptPassword(user) {
        if (user.password) {
            const userWithEncryptedPassword = new user_entity_1.User();
